...
| R8.2 | `已完成（接受現有證據）` | 純本機模式 | 132 項核心測試涵蓋未登入時的主機、SSH、SFTP、本機 Terminal、Serial、Keychain 與資料保存；使用者因 Mac 正在執行其他任務不切換帳號，並於 2026-08-10 明確接受既有自動測試及先前日常純本機操作作為驗收證據 |
| R8.3 | `已完成` | Google 登入與同步 | 2026-08-10：第二台 Mac 沿用既有 Google 登入與端對端金鑰，RC2 先備份本機資料，再安全採用雲端資料、建立主機與密碼雙基線並啟用同步；同步主機可直接使用另一台 Mac 保存的密碼連線 |
| R8.4 | `已完成` | 更新安裝 | 2026-08-10：第二台 Mac 在 RC1 內按「檢查更新…」，成功下載、安裝並重新啟動為 RC2 |
| R8.5 | `已完成` | 資料保留 | 2026-08-10：更新後既有 Google 登入、同步保管庫與狀態仍在；雲端主機與 Keychain 密碼成功還原並可正常 SSH 使用，未發現資料遺失 |
| R8.6 | `已完成` | 同名／舊版本 | 2026-08-10：第二台 Mac 的 RC2 再按「檢查更新…」顯示目前已有最新版本；本機發布檢查另會拒絕版本或 Build 倒退 |
| R8.7 | `已完成` | 更新失敗復原 | 2026-08-10：第二台 Mac 中斷網路後只顯示更新錯誤，App 沒有關閉；恢復網路後正常。遭竄改 ZIP、錯誤簽章、遭竄改 appcast 與無效 XML亦全部被負向測試拒絕 |
| R8.8 | `延後（非阻擋）` | 第二位測試者 | 產品定位以個人使用為主；朋友安裝與回饋移至 `1.0.0` 發布後進行，不作為私人正式版阻擋條件，也不需要提供任何真實主機或密碼 |
...
| 編號 | 狀態 | 執行者 | 任務 | 驗收證據／人工動作 |
|---|---|---|---|---|
| R9.1 | `已完成` | Codex | 將 RC 的已驗證程式設定為 `1.0.0` | 正式版沿用 RC2 已驗證程式碼；tag `v1.0.0` 指向 commit `ad08417915abcfede8ed3b47e154331d261f5a71` |
| R9.2 | `已完成` | Codex | 重新執行全部測試、安全掃描與發布腳本 | 2026-08-10：278 項測試、Release 建置、安全掃描、App／ZIP、Sparkle 雙簽章與五個發布資產驗證全部通過；Build `20260810141610`，ZIP SHA-256 `541562ad5d365ed0a9fc9f4bd1bdc8e5f1a0da42e61f1e85b4469dd07affb77e`。五個資產另從 GitHub Draft 重新下載驗證通過 |
...
`1.0.0` 上線不代表更新鏈已永久完成。`1.0.1` 會先完成兩項已排定功能，再驗證真正的正式版本能收到下一版。兩項功能都涉及密碼或資料刪除，不以「低風險小改」處理，必須具備完整負向測試與第二台 Mac 驗收。

| 編號 | 狀態 | 任務 | 驗收證據／人工動作 |
|---|---|---|---|
| R10.1 | `尚未開始` | 實作主機與群組的端對端加密刪除同步 | 使用 tombstone、連續 revision 與基線判斷；驗證單筆、巢狀群組、離線、兩端衝突及不得誤刪 |
| R10.2 | `尚未開始` | 實作 `sudo`／`su` 安全提示的自動密碼送出 | 僅限目前主機綁定密碼與明確安全提示；新密碼、確認密碼、OTP 及一般 Shell 提示必須拒絕 |
| R10.3 | `尚未開始` | 建立並發布 `1.0.1` Release Candidate | 全部自動測試、第二台 Mac 的刪除傳播與密碼提示人工驗收 |
| R10.4 | `等待人工驗證` | 從正式 `1.0.0` 按「檢查更新」升級 | 更新成功截圖／確認 |
| R10.5 | `尚未開始` | 確認 `1.0.1` 後所有資料與核心功能正常，完成正式更新鏈驗收 | 回歸測試與人工確認；本計劃標記完成 |
...
- 自動測試不是全部通過。
- 依目前已核准的一份私人 iCloud AES-256 備份決策，若該備份遺失、無法解鎖或未通過隔離還原簽署測試，則停止發布；未來有私人外接設備時再補第二份。
- 遭竄改 ZIP 仍能被安裝。
