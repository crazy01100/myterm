...
| R8.6 | `等待人工驗證` | 同名／舊版本 | 本機更新實驗室已驗證相同版本不重複提示；請在第二台 Mac 的 RC2 再按一次「檢查更新…」，應顯示目前已是最新版本。較舊版本與 Build 倒退另由發布前檢查拒絕 |
| R8.7 | `等待人工驗證` | 更新失敗復原 | 2026-08-10 已重跑負向測試：遭竄改 ZIP、錯誤簽章、遭竄改 appcast 與無效 XML 全部被拒絕；尚需第二台 Mac 關閉網路後檢查更新，確認 RC2 仍可開啟與使用 |
