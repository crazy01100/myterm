...
> 2026-08-10 使用者已核准本計劃。R1 至 R7 已完成，RC1 與 RC2 均已通過人工放行、發布、部署與外部驗證，Pages 回復演練亦已完成；Sparkle 私鑰與加密備份均已就緒，GitHub 採私人 repository。更新站使用 `mtus.lieniapp.work`，採 Cloudflare Pages Direct Upload，由 GitHub Actions 自動部署公開成品，但不讓雲端持有 Sparkle 私鑰。R8 已在第二台 Mac 完成 RC1 → RC2 真實更新、安全採用雲端資料、跨裝置同步與密碼解密使用；剩餘純本機及失敗復原等人工情境驗收完成後，才進入正式 `1.0.0`。
...
| R8.2 | `等待人工驗證` | 純本機模式 | 自動測試已涵蓋未登入時的主機、SSH、SFTP、本機 Terminal 與 Serial 設定；為避免碰觸已同步資料，請在同一台 Mac 的全新標準 macOS 使用者帳號開啟 `/Applications/MyTerm.app`，不登入 Google 完成介面操作驗收 |
...
| R8.8 | `等待人工驗證` | 第二位測試者 | 至少一位工程師朋友依安裝說明完成安裝與更新，回報不清楚處；不需要提供任何真實主機或密碼 |
