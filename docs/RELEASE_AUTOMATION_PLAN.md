...
最後更新：2026-08-10
文件狀態：`已核准並執行`
實作狀態：`R8 已完成，R9 正式 1.0.0 準備中`

> 2026-08-10 使用者已核准本計劃。R1 至 R8 已完成，RC1 與 RC2 均已通過人工放行、發布、部署與外部驗證，Pages 回復演練亦已完成；Sparkle 私鑰與加密備份均已就緒，GitHub 採私人 repository。更新站使用 `mtus.lieniapp.work`，採 Cloudflare Pages Direct Upload，由 GitHub Actions 自動部署公開成品，但不讓雲端持有 Sparkle 私鑰。第二台 Mac 已完成 RC1 → RC2 真實更新、安全採用雲端資料、跨裝置同步、密碼解密、相同版本與斷線復原驗收，現進入正式 `1.0.0` Draft 準備。
