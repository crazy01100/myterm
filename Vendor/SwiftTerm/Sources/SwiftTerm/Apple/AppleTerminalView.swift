...
extension TerminalView {
    ...
    func getAttributes (_ attribute: Attribute, withUrl: Bool) -> [NSAttributedString.Key:Any]?
    {
        ...
        #if os(macOS)
        if let transform = displayForegroundTransform {
            fgColor = transform(attribute, fgColor, bgColor)
        }
        #endif
    ...
    func buildAttributedString (row: Int, line: BufferLine, cols: Int) -> ViewLineInfo
    {
        ...
        #if os(macOS)
        let highlight = displayLineHighlight?(terminal, line, cols)
        var lastIsHighlighted = false
        #endif
        ...
        while col < cols {
            ...
            guard var attributes = getAttributes(attr, withUrl: hasUrl) else {
                flushPending()
            ...
            #if os(macOS)
            let isHighlighted = !isSelected && highlight?.columns.contains(col) == true
            if isHighlighted != lastIsHighlighted {
                flushPending()
                lastIsHighlighted = isHighlighted
            }
            if isHighlighted, let highlight {
                attributes[.foregroundColor] = highlight.color
                if attributes[.underlineColor] != nil { attributes[.underlineColor] = highlight.color }
                if attributes[.strikethroughColor] != nil { attributes[.strikethroughColor] = highlight.color }
            }
            #endif
