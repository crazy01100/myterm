...
open class TerminalView: NSView, NSTextInputClient, NSUserInterfaceValidations, TerminalDelegate {
    ...
    /// MyTerm extension: display-only foreground adjustment. Nil preserves upstream behavior.
    /// The callback must not mutate the terminal or view. Called before attribute caching.
    public var displayForegroundTransform: ((Attribute, NSColor, NSColor) -> NSColor)? {
        didSet { colorsChanged() }
    }

    /// MyTerm extension: at most one column range per rendered line. No buffer mutation.
    /// Selection takes precedence; callers should omit TUI / wrapped / hidden content.
    public var displayLineHighlight: ((Terminal, BufferLine, Int) -> (columns: Range<Int>, color: NSColor)?)? {
        didSet { colorsChanged() }
    }
