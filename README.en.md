...
- [Set up your own Firebase sync backend](FIREBASE_SETUP.md)
- [Migrate host metadata from Termius](TERMIUS_MIGRATION.md)
- [System architecture](ARCHITECTURE.md)
- [Security design](SECURITY.md)

The linked guides and update website are currently in Traditional Chinese. This README provides English documentation; the app interface remains in Traditional Chinese. English menu descriptions below refer to the corresponding Chinese controls.
...
- Import MyTerm or Termius host metadata with a selectable preview, and export MyTerm host metadata. See [Termius migration](TERMIUS_MIGRATION.md) for supported data and the optional conversion tool.
...
The official release does not require your own Firebase project. Only developers building from source with their own Google sign-in and sync backend need to follow the [Firebase setup guide](FIREBASE_SETUP.md). Missing cloud configuration does not affect local features.
...
You need macOS 26, Apple Silicon, and Xcode 26 or compatible Command Line Tools.
...
See [DEVELOPMENT.md](DEVELOPMENT.md) for build channels, scripts, candidate builds, and the release process.
...
- The MyTerm app does not read the Termius Vault directly. The repository's optional conversion tool handles only compatible host/group metadata, not passwords or private keys. See [Termius migration](TERMIUS_MIGRATION.md) for the full limitations.
...
See [SECURITY.md](SECURITY.md) and [ARCHITECTURE.md](ARCHITECTURE.md) for storage details and trust boundaries.
