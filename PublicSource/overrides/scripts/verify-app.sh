...
expected_sparkle_public_key="${MYTERM_SPARKLE_PUBLIC_KEY:-}"
if [[ -z "$expected_sparkle_public_key" && -f "$sparkle_public_key_file" ]]; then
    expected_sparkle_public_key="$(/usr/bin/tr -d '\r\n' < "$sparkle_public_key_file")"
fi
if [[ -n "$expected_sparkle_public_key" ]]; then
...
if [[ -n "${MYTERM_SPARKLE_FEED_URL:-}" ]]; then
    actual_feed="$(/usr/bin/plutil -extract SUFeedURL raw "$plist" 2>/dev/null || true)"
    [[ "$actual_feed" == "$MYTERM_SPARKLE_FEED_URL" ]] || { echo "Update feed mismatch." >&2; exit 1; }
fi
