...
本專案只提供原始碼、建置所需資源與文件。MyTerm 的功能取捨以維護者個人工作習慣為主，歡迎自行建置與調整；此處不提供預先打包的 App，也不提供代管同步或更新服務。
...
- 終端字體可用 `⌘＋`／`⌘＝` 放大、`⌘－` 縮小、`⌘0` 還原為 14 點；每次調整 1 點，範圍為 10～32 點。只調整目前操作的窗格，雙窗格可各自保持大小；同一 Session 切換分頁、合併、拆分與原位重連後保留大小，新連線使用預設值。三項動作可在「設定 → 快捷鍵 → 終端機」自訂、停用或恢復預設，預設也支援數字鍵盤的加／減／0。
...
- 可選擇使用 Sparkle 從自己設定的 HTTPS feed 與驗證公鑰進行更新；未設定時，透過更新原始碼並重新建置取得新版。
...
自行建置並啟用 Google 登入與同步時，請依照 [Firebase 自架同步設定](FIREBASE_SETUP.md) 使用自己的 Firebase 專案；未加入雲端設定仍可完整使用本機功能。
