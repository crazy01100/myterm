...
MyTerm.app
  ...
  App updates ───────────── HTTPS ──> your configured HTTPS update service
                                         │ Sparkle appcast
                                         v
...
See [Firebase setup](FIREBASE_SETUP.en.md) for configuration boundaries between local-only source builds and independently configured sync backends, including Firebase/Google Cloud prerequisites.
...
Platform SVGs are copied to `Contents/Resources/PlatformIcons` and loaded through `Bundle.main`. Packaged-resource checks verify the icon set and reject MyTerm resource accessors that depend on the build machine. No App Translocation, read-only image, or nonreplaceable app location should be used for an update installation.
...
| `update-site` | Optional static update-site template |
