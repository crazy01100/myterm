...
The deployment workflow is limited to the original repository; select `main` for manual runs. The job references `production`, whose approval rules and Secrets require separate administrator configuration. If an old tag lacks that binding, redeploy its original release_tag through current main without changing signed assets. See [production deployment protection](../DEVELOPMENT.en.md#production-deployment-protection) for environment setup, repository Secrets migration, and plan limitations.
