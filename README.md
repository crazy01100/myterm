...
- 主機庫左側的「Logs」會記錄互動式 SSH 的連線時間、持續時間、連線當下的主機與帳號端點，以及進行中、完成、失敗、取消或未完整結束等結果；可搜尋、篩選及由使用者清理。Logs 只保存本機連線中繼資料，不記錄輸入命令或終端機輸出。
...
6. 主機庫左側的「Known Hosts」與「Logs」分別開啟 SSH 信任記錄與連線軌跡。Logs 可依主機、端點或結果搜尋與篩選；刪除單筆或清除全部只移除本機稽核紀錄，不會刪除主機、密碼或正在執行的 Session。
...
- Logs 是保存在目前 Mac 的敏感使用軌跡，包含主機名稱、帳號端點、時間與連線結果；不進入主機匯出或 Firebase 同步，也不包含密碼、命令、終端機輸入／輸出或 OpenSSH verbose debug。紀錄上限為 5,000 筆，可由使用者單筆刪除或清除全部。
