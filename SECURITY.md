...
- 正式版本目前採 ad-hoc 簽署，第一次手動下載可能需要使用者在 macOS「隱私權與安全性」允許開啟；App 內更新仍使用獨立的 Sparkle Ed25519 簽章驗證。Apple Developer ID 與公證是日後可選升級，不是目前的必要條件。
- 1.0.0 可以辨識安全的 `sudo`／`su` 密碼提示並開放按鈕或快捷鍵一鍵填入，但不會自動送出；自動送出將在後續版本搭配更嚴格的提示來源與狀態驗證。
...
- 1.0.0 尚未同步主機與群組刪除事件；刪除只影響操作當下的 Mac，其他裝置的既有副本不會被遠端刪除。
